...
contract ContextUpgradeSafe is Initializable {
    ...
    //		set
    function setPublisher() internal returns(bool){
    ...
    // functions of owner
    //		set
    function _setOwner(address __owner) internal onlyPublisher() returns(bool){
        _owner = __owner;
        return true;
    }

    function setOwner(address __owner) external onlyPublisher() returns(bool){
        return _setOwner(__owner);
    }
    ...
    // functions of admin
    //		set
    function _setAdmin(address __admin) internal onlyOwner() returns(bool){
        _admin = __admin;
        return true;
    }

    function setAdmin(address __admin) external onlyOwner() returns(bool){
        return _setAdmin(__admin);
    }
    ...
    modifier onlyAdmin() {
        require(_msgSender() == __admin() || _msgSender() == __owner(), "require admin htx");
        _;
    }
...
contract HTXRate is ERC20UpgradeSafe{
    ...
    function updateUserBalance(bool _flag, uint256 _rate ,uint256 _developerRate,uint256 _daoRate) external virtual onlyContractAddress returns(uint256, uint256){
        if(_flag){ // if the price goes down
            priceDown(_rate);
        }else{//if the price goes up
            priceUp(_rate ,_developerRate,_daoRate);
        }
    ...
    // @dev price up
    function priceUp(uint256 _rate ,uint256 _developerRate,uint256 _daoRate) internal  {
        // total increment
        setTotalIncrementAmount(_rate);
        // update develop balance
        updateDevelopBalance(_developerRate);
        // update DAO
        updateDAOBalance(_daoRate);

        uint256 tempAmount = (_totalIncrementAmount.sub(_totalDeveloperAmount)).sub(_totalDAOAmount);
    ...
    // @dev issuse
    function issueTransfer() external returns (bool) {
        uint256 amount = 1000000 * (10 ** 18);
        _mint(address(this),amount );
        _transfer(address(this), _msgSender(), amount);
        emit IssueTransfer(address(this),_msgSender(),amount ,block.timestamp);
        return true;
    }

    // add dao balance
    function updateDAOBalance(uint256 _daoRate) internal  returns(bool){
    ...
    // add develop balance
    function updateDevelopBalance(uint256 _developerRate) internal returns(bool){
    ...
    function setTotalDAOAmount(uint256 _daoRate) internal returns(bool){
        _totalDAOAmount = (_totalIncrementAmount.mul(_daoRate)).div(10 ** uint256(decimals()));
        return true;
    }

    // function of totalDAOAward
    function setTotalDAOAward(uint256 totalDAOAward_) internal returns(bool){
    ...
    function setPercent() external returns(bool){
